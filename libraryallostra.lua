...
local requestsDisabled = true --getgenv and getgenv().DISABLE_Allostra_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Version 1"
local AllostraFolder = "ALLOSTRA"
local ConfigurationFolder = AllostraFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		AllostraOpen = {Type = 'bind', Value = 'K', Name = 'Allostra Keybind'},
		-- buildwarnings
		-- Allostraprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.AllostraOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(AllostraFolder) then
				if isfile and isfile(AllostraFolder..'/settings'..ConfigurationExtension) then
					file = readfile(AllostraFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"AllostraOpen":{"Value":"K","Type":"bind","Name":"Allostra Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Allostra Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Allostra had an issue accessing configuration saving capability.')
		end
	end
end
...
local Allostra = useStudio and script.Parent:FindFirstChild('Allostra') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local AllostraDestroyed = false -- True when AllostraLibrary:Destroy() is called

repeat
	if Allostra:FindFirstChild('Build') and Allostra.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Allostra | Build Mismatch')
		print('Allostra may encounter issues as you are running an incompatible interface version ('.. ((Allostra:FindFirstChild('Build') and Allostra.Build.Value) or 'No Build') ..').\n\nThis version of Allostra is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Allostra = Allostra, useStudio and script.Parent:FindFirstChild('Allostra') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

Allostra.Enabled = false

if gethui then
	Allostra.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Allostra)
	Allostra.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Allostra.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Allostra.Name and Interface ~= Allostra then
			Interface.Enabled = false
			Interface.Name = "Allostra-Old"
		end
	end
elseif not useStudio then
...
if Allostra.AbsoluteSize.X < minSize.X and Allostra.AbsoluteSize.Y < minSize.Y then
...
local Main = Allostra.Main
local MPrompt = Allostra:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Allostra:FindFirstChild('Drag')
...
Allostra.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Allostra/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = Allostra.Notifications

local SelectedTheme = AllostraLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AllostraLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Allostra.Main.BackgroundColor3 = SelectedTheme.Background
	Allostra.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Allostra.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Allostra.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Allostra.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Allostra.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Allostra.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Allostra.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Allostra.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Allostra:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Allostra | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Allostra | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Allostra had an issue decoding the configuration file, please try delete the file and reopen Allostra.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(AllostraLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Allostra | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AllostraLibrary:Notify({Title = "Allostra Flags", Content = "Allostra was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AllostraLibrary.Flags) do
...
function AllostraLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Allostra | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AllostraLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			AllostraLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "AllostraOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(AllostraFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Allostra Settings', 0, true)

	if TabList['Allostra Settings'] then
		TabList['Allostra Settings'].LayoutOrder = 1000
	end

	if Elements['Allostra Settings'] then
		Elements['Allostra Settings'].LayoutOrder = 1000
	end
...
function AllostraLibrary:CreateWindow(Settings)
	print('creating window')
	if Allostra:FindFirstChild('Loading') then
		if getgenv and not getgenv().AllostraCached then
			Allostra.Enabled = true
			Allostra.Loading.Visible = true

			task.wait(1.4)
			Allostra.Loading.Visible = false
		end
	end

	if getgenv then getgenv().AllostraCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AllostraLibrary:Notify({Title = 'Build Mismatch', Content = 'Allostra may encounter issues as you are running an incompatible interface version ('.. ((Allostra:FindFirstChild('Build') and Allostra.Build.Value) or 'No Build') ..').\n\nThis version of Allostra is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			overrideSetting("General", "AllostraOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind must be a KeyCode enum")
			overrideSetting("General", "AllostraOpen", keybind.Name)
		else
			error("ToggleUIKeybind must be a string or KeyCode enum")
		end
	end

	if isfolder and not isfolder(AllostraFolder) then
		makefolder(AllostraFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Allostra"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Allostra Interface Suite" then
		LoadingFrame.Version.Text = "Allostra UI"
	end
	...
	if not Settings.DisableAllostraPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				AllostraLibrary:Notify({
					Title = "Allostra Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(AllostraFolder.."/Discord Invites") then
			makefolder(AllostraFolder.."/Discord Invites")
		end

		if isfile and not isfile(AllostraFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(AllostraFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Allostra RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(AllostraFolder.."/Key System") then
			makefolder(AllostraFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Allostra | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Allostra specific development.')
				end
			end
		end
		...
		if isfile and isfile(AllostraFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(AllostraFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(AllostraFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						AllostraLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called AllostraLibrary:Destroy()
				if AllostraDestroyed then
					return
				end
				if not Success then
					...
					print("Allostra | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Allostra specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AllostraLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AllostraLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Allostra.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Allostra | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Allostra specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AllostraLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Allostra | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Allostra specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AllostraLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Allostra | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Allostra specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AllostraLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AllostraLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Allostra | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Allostra specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AllostraLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Allostra | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Allostra specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AllostraLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AllostraLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AllostraLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Allostra had an issue creating settings.') end
...
function AllostraLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AllostraLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function AllostraLibrary:Destroy()
	AllostraDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Allostra Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "AllostraOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function AllostraLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AllostraLibrary:Notify({Title = "Allostra Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AllostraLibrary:Notify({Title = "Allostra Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Allostra Configurations Error | '..tostring(result))
			AllostraLibrary:Notify({Title = "Allostra Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = AllostraLibrary:CreateWindow({
	--	Name = "Allostra Example Window",
	--	LoadingTitle = "Allostra Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using Allostra may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Allostra to get the key from
	...
	----AllostraLibrary:Notify({Title = "Allostra Interface", Content = "Welcome to Allostra. These - are the brand new notification design for Allostra, with custom sizing and Allostra calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(AllostraLibrary.Theme) do
...
task.delay(4, function()
	AllostraLibrary.LoadConfiguration()
